...
const ChatCard = ({ image1, image2, image3, name, desc, subname, date, setInboxState, inboxState, setLoading }) => {
  return (
    <div className="flex w-full gap-5 pb-4 border-b-[1px] border-black hover:cursor-pointer" onClick={() => {
      ...
      {image3 ? (
        <div className="flex relative w-[40px] justify-center items-center">
          <div className="w-[30px] h-[30px] bg-[#2F80ED] rounded-full flex justify-center items-center text-white">
            {image3}
          </div>
        </div>
      ) : (
      ...
      <div className="flex flex-col">
        ...
        {subname &&
        <div className="text-sm text-[#4F4F4F] font-bold">{subname}</div>
        }
